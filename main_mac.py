...
for scn, xy in scenario_data.items():
    ...
    history = build_train_birnn_with_attention(x_train, x_test, y_train, y_test, epochs=1, use_GPU=False)
    ...
    # test the ANN
    history = build_train_ann(x_train, x_test, y_train, y_test, epochs=1)
    plot_train_history(history, note=str(scn) + ' ANN')
    eval = history.model.evaluate(x=x_test, y=y_test)
    scenario_train_histories[('ANN', scn)] = [history, eval]

    # test the RNN
    history = build_train_rnn(x_train, x_test, y_train, y_test, epochs=1, use_GPU=False)
    plot_train_history(history, note=str(scn) + ' RNN')
    eval = history.model.evaluate(x=x_test, y=y_test)
    scenario_train_histories[('RNN', scn)] = [history, eval]

    # test the CNN
    history = build_train_cnn(x_train, x_test, y_train, y_test, epochs=1)
